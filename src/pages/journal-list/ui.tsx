    ...
    return (
      <div className="w-full h-full flex flex-col bg-tg-bg text-tg-text p-4 pb-tg-safe-bottom">
        ...
        <div className="flex flex-col gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-20 bg-tg-secondary-bg animate-pulse rounded-2xl" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full h-full flex flex-col bg-tg-bg text-tg-text pb-tg-safe-bottom overflow-y-auto">
      ...
      <div className="p-4 bg-tg-bg shrink-0">
        ...
        {streak > 0 && (
          <p className="text-[13px] text-tg-accent font-semibold mt-1">
      ...
      <div className="flex items-center gap-2 overflow-x-auto px-4 py-3 shrink-0 scrollbar-none">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-[13px] font-semibold transition-all shrink-0 ${
            filter === 'all' ? 'bg-tg-accent text-white' : 'bg-tg-secondary-bg text-tg-text'
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-[13px] font-semibold transition-all shrink-0 ${
            filter === 'month' ? 'bg-tg-accent text-white' : 'bg-tg-secondary-bg text-tg-text'
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-[13px] font-semibold transition-all shrink-0 ${
            filter === 'lowMood' ? 'bg-tg-accent text-white' : 'bg-tg-secondary-bg text-tg-text'
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-[13px] font-semibold transition-all shrink-0 ${
            filter === 'highMood' ? 'bg-tg-accent text-white' : 'bg-tg-secondary-bg text-tg-text'
          }`}
        >
        ...
        {filteredEntries.length === 0 ? (
          ...
          Object.entries(groupedEntries).map(([monthStr, monthEntries]) => (
            <div key={monthStr} className="flex flex-col gap-3">
              <h3 className="text-[14px] font-bold text-tg-hint uppercase tracking-wider ml-1">
                {monthStr}
              </h3>
                ...
                {monthEntries.map((entry) => {
                  ...
                  return (
                    <div
                      ...
                      className="bg-tg-section border border-tg-hint/10 rounded-2xl p-4 shadow-sm flex items-center gap-4 cursor-pointer hover:opacity-[0.98] active:scale-[0.99] transition-all"
                    >
                      {/* Date Indicator */}
                      <div className="flex flex-col items-center justify-center bg-tg-secondary-bg border border-tg-hint/15 rounded-xl w-12 h-12 shrink-0">
                        <span className="text-[17px] font-bold leading-none">{dayNum}</span>
                        <span className="text-[9px] font-semibold text-tg-hint uppercase mt-0.5">{weekday}</span>
                      </div>

                      {/* Content middle */}
                      <div className="flex-1 min-w-0">
                        <p className="text-[14px] text-tg-text font-medium line-clamp-2 leading-relaxed">
                          {entry.free_text ? entry.free_text.replace(/###.*?\n/g, '') : ''}
                        </p>
                        <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                          {entry.mood !== undefined && (
                            <span className="text-[11px] font-bold text-tg-accent flex items-center gap-0.5">
                              <span>😊</span>
